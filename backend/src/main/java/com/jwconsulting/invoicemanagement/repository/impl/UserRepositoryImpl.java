...
//            jdbc.update(DELETE_USER_FROM_PASSWORD_VERIFICATION_QUERY), Map.of("id", user.getId());  //click the link once and verify your password. click it again then the link will not be valid.
    ...
    @Override
    public void updateUserAccountSettings(Long userId, Boolean enabled, Boolean notLocked) {
        try {
            jdbc.update(UPDATE_USER_ACCOUNT_SETTINGS_QUERY, Map.of("userId", userId, "enabled", enabled, "notLocked", notLocked));
        } catch (Exception e) {
            log.error(e.getMessage());
            throw new ApiException("An error occurred. Please try again.");
        }
