...
/**
 * In production, instead of checking for key words in the url, check for the entire url pattern.
 * This ensures the upmost security with the interceptor and applicaton so that no one can trick the system.
 **/
  intercept(request: HttpRequest<unknown>, next: HttpHandler): Observable<HttpEvent<unknown>> | Observable<HttpResponse<unknown>> {
    if(request.url.includes('verify') || request.url.includes('login') || request.url.includes('register') || request.url.includes('refresh') || request.url.includes('reset')) {
      return next.handle(request);  //let the request continue it's course
    }
  ...
  if(!this.isTokenRefreshing) {
    ...
    return this.userService.refreshToken$().pipe(
      switchMap((response) => {
        ...
        console.log('New token recieved: ', response.data.access_token);
